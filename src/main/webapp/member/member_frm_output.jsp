...
<%
	...
	//비연결성인 웹에서 한정적인 자우너을 먼저 선점하는 접속자 이외에는 "해당자원을 이미 다른 접속자가
	//사용중입니다." 를 제공해야한다
	boolean flag= ms.searchId(mDTO.getId());
	
	if(!flag){
	//boolean result = ms.addMember(mDTO);
...
<div style="margin: 0px auto; background: #FFFFFF url('images/member_success.png')no-repeat; position: relative;width: 606px; height: 378px;">
...
<a href="javascript:location.replace('http://192.168.10.76/jsp_prj/index.html')" class="btn btn-secondary btn-sm">메인으로 </a>
</div>
</div>
</div>
</c:when>
...
<a href="http://192.168.10.76/jsp_prj/index.html">메인화면</a>
