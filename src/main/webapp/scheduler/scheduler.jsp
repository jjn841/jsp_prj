...
.sunTitle{ background-color: #FF0000; color: #FFFFFF;}
/*.weekTitle{}*/
...
<div id="calendarWrap">
<%
	Calendar cal=Calendar.getInstance();//시스템의 현재날짜 정보를 갖고 있다.
	...
	if(paramYear != null){//입력되는 년, 월을 사용하여 다른 날짜로 설정
		...
		cal.set(Calendar.MONTH, Integer.parseInt(paramMonth)-1);//2월
	}//end if
...
<%-- <a href="scheduler.jsp?year=<%=nowMonth-1==0? nowYear-1 : nowYear%>&month=<%= nowMonth-1 == 0?12:nowMonth-1 %>"><span title="이전 월" class="prevMonth">
&lt;</span></a>
<span class="mainDate"><%=nowYear %>.<%=nowMonth %></span>
<a href="scheduler.jsp?year=<%=nowMonth+1==13? nowYear+1 : nowYear%>&month=<%=nowMonth+1==13?1:nowMonth+1%>">
<span title="다음 월" class="nextMonth">&gt;</span></a>
<span title="오늘" class="btnToDay">
<a href="scheduler.jsp?">오늘</a>
</span> --%>
<a href="javascript:moveCalendar('p',<%= nowYear %>,<%= nowMonth%>)"><span title="이전 월" class="prevMonth">
&lt;</span></a>
<span class="mainDate"><%=nowYear %>.<%=nowMonth %></span>
...
<% //1일의 요일 - 1일을 출력하기전 공백 설정 
	...
	for (int tempDay=1; ;tempDay++){
		...
		//설정된 날짜로 얻낸 날짜와 임시로 증가하는 날짜가 다르면 (다음달 1일 )
			System.out.println("----"+(cal.get(Calendar.MONTH)+1) + "---"+tempDay);
