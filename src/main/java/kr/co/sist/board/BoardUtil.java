...
public class BoardUtil {
	 ...
	 * url-이동할 URL, field-검색 필드 ( 제목, 내용, 작성자), keyword-검색 키워드
	 * 를 pDTO로 입력 받아서 <br>
	...
	public static String pagination( PaginationDTO pDTO ) {
		...
		//int pDTO.getPageNumber()=3;//한화면에 보여줄 페이지 인덱스의 수
		//2. 화면에 보여줄 시작페이지 번호
		int startPage = ((pDTO.getCurrentPage()-1)/pDTO.getPageNumber())*pDTO.getPageNumber()+1;//1,2,3페이지는 => 1
		//3. 화면에 보여줄 마지막 번호
		...
		if( pDTO.getCurrentPage() > pDTO.getPageNumber()){//시자게이지보다 1적은 페이지로 이동
		...
		while ( movePage <= endPage ){//
			if( movePage == pDTO.getCurrentPage() ){//현재페이지는 링크 설정 x 
